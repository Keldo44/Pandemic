...
public class g_archivos2 {
	...
	/*
	 * public  int[] encontrarRuta(int[][] colind, int a, int b) { Ciudad
	 * ciudad = new Ciudad(colind, a, b);
	 * 
	 * List<Integer> arrayList = ciudad.shortestPath(); int[] array = new
	 * int[arrayList.size()]; for (int i = 0; i < arrayList.size(); i++) { array[i]
	 * = arrayList.get(i); } int[] ruta = array.clone(); return ruta; }
	 */
	...
	public g_archivos2(String file) {
		...
		fill_ciudades();
		fill_colind();
		imprimir_cities(data);
		imprimirMatriz(colind);
		fill_coords();
	}
}
